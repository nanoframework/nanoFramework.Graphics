...
namespace nanoFramework.Presentation
{
    ...
    internal class LayoutManager : DispatcherObject
    {
        ...
        public void UpdateLayout()
        {
            ...
            //infinite loop.   In the nanoCLR we will probably have better ways of
        ...
        private bool LimitExecution(ref TimeSpan loopStartTime)
        {
            ...
            {
                // from Netmf -- loopStartTime = Microsoft.SPOT.Hardware.Utility.GetMachineTime();
                loopStartTime = tsNow;
            }
            else
            {
                // from Netmf -- if ((Microsoft.SPOT.Hardware.Utility.GetMachineTime() - loopStartTime).Ticks > 153 * 2 * TimeSpan.TicksPerMillisecond) // 153*2 = magic*science
                if ((tsNow - loopStartTime).Ticks > 153 * 2 * TimeSpan.TicksPerMillisecond) // 153*2 = magic*science
                {
